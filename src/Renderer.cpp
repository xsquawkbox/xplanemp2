...
GLfloat	Render_LabelViewport[4] = { 0.0, 0.0, 0.0, 0.0 };
...
Render_PrepLists()
{
	...
	CullInfo::GetCurrentViewport(Render_LabelViewport);
...
/*
 * RenderingCallbacks
 * - rather than the wasteful overridable rendering callback arrangement, we have
 *   one set of three callbacks.
 *
 *   * aircraft		[Phase_Airplanes]
 *   * labels		[Phase_Window]
 *
 *   The aircraft phase can be completely elided if there are no LegacyCSL
 *   objects loaded.
 */
...
static int
XPMP_RenderCallback_Labels(XPLMDrawingPhase, int, void *)
{
#if 0
	bool is_blend = true;
	if (gPlaneRenderTypeRef) {
	...
	CullInfo::GetCurrentViewport(curViewport);
	...
	glPushMatrix();
	glLoadIdentity();
	glOrtho(0, Render_LabelViewport[2], 0, Render_LabelViewport[3], -1, 1);
	...
	for (const auto &label: gLabelList) {
		...
		// to rescale into the correct coordinate system... (x-left) / width * newWidth + newleft
		float lx = (label.x - Render_LabelViewport[0]) / Render_LabelViewport[2] * curViewport[2] + curViewport[0];
		float ly = (label.y - Render_LabelViewport[1]) / Render_LabelViewport[3] * curViewport[3] + curViewport[1];

		XPLMDrawString(c, lx, ly+10, (char *) label.labelText.c_str(), nullptr, xplmFont_Basic);
	}
...
Renderer_Attach_Callbacks()
{
	...
	XPLMRegisterDrawCallback(&XPMP_RenderCallback_Labels,
		xplm_Phase_Window, 1, nullptr);
...
Renderer_Detach_Callbacks()
{
	InstanceCompat_Stop();
	TCAS::DisableHooks();
	XPLMUnregisterDrawCallback(&XPMP_RenderCallback_Labels,
		xplm_Phase_Window, 1, nullptr);
